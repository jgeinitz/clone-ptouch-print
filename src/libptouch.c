...
struct _pt_dev_info ptdevs[] = {
  ...
  /* 180dpi, 128px, maximum tape width 24mm, must send TIFF
     compressed pixel data */
  {0x04f9, 0x2011, "PT-2450PC", 128, 180, FLAG_RASTER_PACKBITS},
  {0x04f9, 0x2019, "PT-1950", 128, 180, FLAG_RASTER_PACKBITS},
  /* 180dpi, apparently 112px printhead ?, maximum tape width 18mm -
     unconfirmed if it works */
  {0x04f9, 0x201f, "PT-2700", 128, 180, FLAG_NONE},
  {0x04f9, 0x202c, "PT-1230PC", 128, 180, FLAG_NONE},
  /* 180dpi, supports tapes up to 12mm - I don't know how much
     pixels it can print! */
  ...
  /* 180dpi, maximum 128px, max tape width 24mm - reported to work
     with some quirks */
  /* Notes about the PT-2730: was reported to need 48px whitespace
     within png-images before content is actually printed -
     can not check this */
...
int ptouch_open(ptouch_dev *ptdev)
{
  ...
  while ((dev=devs[i++]) != NULL) {
    ...
    for (int k=0; ptdevs[k].vid > 0; k++) {
      if ((desc.idVendor == ptdevs[k].vid) &&
	  (desc.idProduct == ptdevs[k].pid) &&
	  (ptdevs[k].flags >= 0)) {
	...
	if (ptdevs[k].flags & FLAG_PLITE) {
	  ...
	  printf("Turn off P-Lite mode by changing switch "
		 "from position EL to position E\n");
	  printf("or by pressing the PLite button for ~ "
		 "2 seconds (or consult the manual)\n");
	  return -1;
	}
	if (ptdevs[k].flags & FLAG_UNSUP_RASTER) {
	  printf("Unfortunately, that printer currently is "
		 "unsupported (it has a different raster data transfer)\n");
	  return -1;
	}
  ...
  fprintf(stderr, _("No P-Touch printer found on USB "
		    "(remember to put switch to position E)\n"));
  libusb_free_device_list(devs, 1);
  return -1;
}
