...
int print_img(ptouch_dev ptdev, gdImage *im)
{
  ...
  d=(gdImageRed(im,1)+gdImageGreen(im,1)+gdImageBlue(im,1) <
     gdImageRed(im,0)+gdImageGreen(im,0)+gdImageBlue(im,0))?1:0;
...
int find_fontsize(int want_px, char *font, char *text)
{
  ...
  for (int i=4; ; i++) {
    if (gdImageStringFT(NULL, &brect[0], -1, font, i, 0.0, 0, 0, text)
	!= NULL) {
      break;
    }
...
int needed_width(char *text, char *font, int fsz)
{
  ...
  if (gdImageStringFT(NULL, &brect[0], -1, font, fsz, 0.0, 0, 0, text)
      != NULL) {
    return -1;
  }
...
gdImage *render_text(char *font, char *line[], int lines, int tape_width)
{
  ...
  int max_height=0;
  for (i=0; i<lines; i++) {
    if ((p=gdImageStringFT(NULL, brect, -black,
			   font, fsz, 0.0, 0, 0, line[i])) != NULL) {
  ...
  for (i=0; i<lines; i++) {
    int ofs=get_baselineoffset(line[i], font_file, fsz);
    int pos=((i)*(tape_width/(lines)))+(max_height) - ofs -5;
    if (debug) {
      printf("debug: line %i pos=%i ofs=%i\n", i+1, pos, ofs);
    }
    if ((p=gdImageStringFT(im, brect, -black,
			   font, fsz-5, 0.0, 0, pos +5, line[i])) != NULL) {
      printf(_("error in gdImageStringFT: %s\n"), p);
    }
  }
  return im;
...
void usage(char *progname)
{
  ...
  printf("\t--writepng <file>\tinstead of printing, "
  ...
  printf("\t\t\t\tIf the text contains spaces, "
	 "use quotation marks\n\t\t\t\taround it.\n");
...
int main(int argc, char *argv[])
{
  ...
  for (i=1; i<argc; i++) {
	...
	usage(argv[0]);
      ...
      printf(_("maximum printing width for this tape is %ipx\n"), tape_width);
      printf("media type = %s\n", ptouch_media_type(ptdev->status->media_type));
      printf("media width = %d mm\n", ptdev->status->media_width);
      printf("tape color = %s (%02X)\n",
	     ptouch_tape_color(ptdev->status->tape_color),
	     ptdev->status->tape_color);
      printf("text color = %s (%02X)\n",
	     ptouch_tape_color(ptdev->status->text_color),
